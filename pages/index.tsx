  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-10">
      ...
      <div className="flex gap-2 mb-8">
        ...
        />
        <button onClick={handleSearch} className="bg-blue-600 text-white px-4 py-2 rounded-lg shadow">
          Search
        </button>
      </div>
        ...
        <div className="w-full max-w-xl bg-white shadow-2xl rounded-2xl p-6 mb-8">
          ...
          <div className="mt-2">
            ...
            <div className="flex gap-2 mt-1">
              {wallet.tags.map((tag) => (
                <span key={tag} className="bg-purple-200 text-purple-800 px-2 py-1 rounded-xl text-sm">
                  {tag}
                </span>
              ))}
            </div>
      ...
      {clusters.length > 0 && (
        <div className="w-full max-w-xl bg-white shadow-md rounded-2xl p-6">
          <h3 className="text-lg font-semibold mb-2">Clustered Wallets (Entity Resolution)</h3>
          <ul className="list-disc list-inside">
            {clusters.map((wallet) => (
              <li key={wallet.address}>
                {wallet.address} — Similarity: {wallet.similarity}
              </li>
            ))}
          </ul>
        </div>
